...
- `csvkit/cli.py` — argument parsing (`argparse`, stdlib) and I/O glue: opens the input
  ...
  e.g. `csvkit ... \| head`, is the reader's choice, not csvkit's failure) — **the write
  to stdout must be immediately followed by `sys.stdout.flush()`, inside the same `try`,
  or `BrokenPipeError` will not reliably be raised at all for ordinary-sized output**
  (verified §3 decision 16: without the flush, a ~64KB-or-smaller write succeeds into
  Python's stdout buffer, and the broken pipe only surfaces during interpreter shutdown,
  outside any `except` block, as a leaked "Exception ignored" message and exit code
  120 — not this decision's promised exit 1 with empty stderr); and one
...
| Output piped into a truncating consumer on a **large** fixture (`\| head -c N`) produces no stderr message and a clean exit — not a raw `BrokenPipeError` traceback or "Exception ignored" spam | Run `python3 -m csvkit csv2json <large-fixture> \| head -c 5`, inspect real captured stderr (must be empty) and the exit code of the left side of the pipe (§3 decision 16). |
| Output piped into an **immediately-closing** reader on an **ordinary small** fixture (a realistic 2-3 row CSV, `\| true`) produces the *same* clean result — **this is the case that actually exercises `sys.stdout.flush()`** (a large-output test can pass by accident, since the write itself may synchronously hit the OS pipe regardless of flush) | Run `python3 -m csvkit csv2json small.csv \| true`, inspect real captured stderr (must be empty, not "Exception ignored...") and confirm exit code 1, **not** 120 (§3 decision 16). |
...
| 9 | whole-funnel-sweep (every doc, not just the 3 already fixed) + content-final-pass (another independent implementation + a pairwise 16-decision contradiction check) | **CLEAN — first genuinely clean round of the cadence** (streak = 1). Both lenses reported `clean:true`; only 5 minors surfaced, none blocking: `docs/PRD.md` carried the same stale `keelwright`-guess path `docs/PROJECT_RULES.md` had already fixed elsewhere (the sweep that fixed one copy didn't re-grep for sibling copies — a one-canonical-location lapse); `docs/ROADMAP.md` and `docs/README.md` both linked a nonexistent `archive/` as unmodified template boilerplate; `docs/lessons-learned/` had never been exercised despite 8 rounds of genuinely lesson-worthy material; and the round-8 carry-forward checklist's own wording ("missed one twice") was internally confusing. The content-final-pass lens's independent implementation found zero new runtime defects, and its pairwise decision-contradiction check found none among all 16 settled §3 decisions. | **clean** (minors dispositioned, fold-in) — fixed the stale PRD.md citation; removed both dead `archive/` references with an honest "nothing archived yet" note; wrote a real dated lessons-learned entry distilling the 5 most durable lessons from this cadence (exception-design pitfalls + the front-door-currency process lesson) and linked it from the index; fixed the confusing checklist wording. **Streak is now 1 of 2 needed** — round 10 is the genuine attempt at the second consecutive clean round. |
| 10 | implement-final (yet another independent from-scratch build) + doc-model-final (fresh full-funnel currency re-check) | **1 blocker + 1 major + 2 minor — streak reset to 0.** The blocker is the sharpest finding of the entire cadence: round 5's `BrokenPipeError` fix (decision 16) cited Python's official SIGPIPE recipe but omitted its `sys.stdout.flush()` line — without it, `sys.stdout.write()` on ordinary-sized output (verified: up to ~64KB) succeeds into Python's buffer without ever raising `BrokenPipeError`; the broken pipe only surfaces during interpreter shutdown, **outside any `except` block**, leaking "Exception ignored in: ..." and exit code **120** — not the promised exit 1 with empty stderr. This survived 4 implementation rounds (5-9) because every test of it used a large fixture, which happens to force a synchronous write regardless of the missing flush; a small, ordinary fixture piped into `\| true` was never tried until round 10. The major: `docs/NEXT-STEPS.md` — the resume cursor, and one of the three front-door surfaces the round-8 carry-forward checklist exists to protect — was not reconciled in round 9's own commit, the exact recurring pattern the checklist was written to prevent, recurring anyway one round later. Two minors: `docs/README.md`'s source-of-truth table had a second, separate dead `archive/` link round 9 missed (only the prose "## Archive" section below it was fixed). | **fail** — fixed: added the missing `sys.stdout.flush()` requirement to §3 decision 16 and §4.1's prose, with the exact failure mechanism (buffered write → shutdown-time leak → exit 120) documented as the reason it's load-bearing, not optional; added a §7 row testing a *small* fixture through an immediately-closing reader specifically (the large-fixture row alone was insufficient and is now explicitly labeled as such); reconciled `docs/NEXT-STEPS.md` with rounds 9-10's actual outcome; de-linked the second dead `archive/` reference; appended two more lessons to the dated lessons-learned entry (checklists reduce but don't eliminate misses; verification fixture choice can silently avoid the code path a fix depends on). **Streak reset to 0 — round 11 restarts the 2-consecutive-clean attempt.** |
| 11 | implement-and-verify-the-verifiers (specifically hunting for §7 rows that could pass without exercising their claimed fix — the exact class round 10 found) + doc-funnel-final (front-door + citation currency) | **CLEAN — zero findings of any kind from either lens, not even a minor.** The first fully spotless round of the entire cadence. Streak = 1 of 2 needed. | **clean** — nothing to fix. Round 12 is the confirming attempt at the second consecutive clean round. |
| 12 | implement-final-confirm (one more independent from-scratch build) + doc-funnel-final-confirm (final full-funnel currency check) | **CLEAN — zero findings of any kind from either lens.** Second consecutive spotless round. | **CONVERGED.** Floor of 3 vastly exceeded (12 rounds run); the last two rounds (11, 12) are both clean. Per `core/03-REVIEW-GATES.md` §2, the cadence formally exits here. **SPEC.md is sealed as implementation-ready.** |

### Outcome

**Converged after 12 rounds.** 8 rounds (1–6, 8, 10) found real, verified defects — 3 of
them blockers (an incomplete exception catalog found and re-found across rounds 1/3, a
blank-data-row misclassification, and a `BrokenPipeError` handling gap that survived 4
implementation attempts because every test used a large fixture). 4 rounds (7, 9, 11, 12)
found only front-door-currency issues or nothing at all. The last 2 rounds are the first
completely spotless pair of the entire cadence.
